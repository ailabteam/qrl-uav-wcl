...
# --- In thông tin cơ bản ---
print("="*60)
print("             BẮT ĐẦU KIỂM TRA MÔI TRƯỜNG DỰ ÁN QRL-UAV")
print("="*60)
...
try:
    ...
    # Thực hiện phép tính nhỏ trên GPU
    ...
    sys.exit(1) # Dừng nếu PyTorch có vấn đề
...
try:
    ...
    # Ưu tiên sử dụng 'lightning.gpu' vì nó được tối ưu hóa cao
    try:
        dev = qml.device("lightning.gpu", wires=1)
        print(f"   - [THÀNH CÔNG] Đã khởi tạo thiết bị 'lightning.gpu' thành công.")
    except qml.DeviceError as e:
        print(f"   - [CẢNH BÁO] Không thể khởi tạo 'lightning.gpu': {e}")
        print("                Thử lại với thiết bị 'default.qubit'. Hiệu năng có thể thấp hơn.")
        dev = qml.device("default.qubit", wires=1)
        print(f"   - [THÀNH CÔNG] Đã khởi tạo thiết bị 'default.qubit'.")

    # Định nghĩa một mạch lượng tử đơn giản với backend là PyTorch
    ...
    # Chuẩn bị dữ liệu đầu vào trên GPU
    ...
    # Kiểm tra quá trình lan truyền ngược
    result_gpu.backward()
    print(f"   - [THÀNH CÔNG] Lan truyền ngược qua mạch lượng tử thành công.")
